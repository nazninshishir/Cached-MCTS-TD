...
class XCom():
    ...
    # positive reward is good for Player 1, negative reward is good for Player -1
    def getReward(self):
        myPlayers = 0
        oppoPlayers = 0
        for i in range(len(self.board)):
    ...
    # positive number good for Player 1, negative number good for Player -1
...
if __name__=="__main__":
    ...
    searcher = mctstd.mctstd(iterationLimit=100)
    searcher2 = cachedmctstd.cachedmctstd(iterationLimit=100)
    ...
    for rounds in range(0,50):
        ...
        for turn in range(1,20):
            ...
            if rounds % 2 == 1 and turn == 1: # two players alternate starting first
                ...
                f = open("exp-cachedmctstd-mctstd-reward.csv", "a")
            ...
            #searcher.showStates()
            searcher2.update(curStateString, nextStateString, reward)
            ...
            actionPkg = searcher2.search(initialState=state, needDetails=True)
            print(actionPkg)    
            action = actionPkg.get('action')
            mcts_reward = actionPkg.get('expectedReward')
            
            f = open("exp-cachedmctstd-mctstd-reward-mctstd.csv", "a")
            f.write("Reward," + str(mcts_reward))
            f.write('\n')            
            f.close()
            ...
            reward = (state.calculatePos() - curPos) * 10
            searcher2.update(nextStateString, nextStateString, reward)
            #searcher.showStates()
            searcher.update(curStateString, nextStateString, reward)
            if state.isTerminal():
                break
        ...
        if rounds % 10 == 9:
            ...
            f = open("exp-cachedmctstd-mctstd.csv", "a")
